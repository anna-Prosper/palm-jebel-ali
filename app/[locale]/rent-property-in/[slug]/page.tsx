...
import { I18N_RENT } from "@/lib/content/i18n/registry";
...
export function generateStaticParams() {
  ...
  for (const slug of Object.keys(I18N_RENT))
    for (const l of LOCALES) if (I18N_RENT[slug][l]) out.push({ locale: l, slug });
  return out;
}
...
export default async function LocalizedRentPage({ params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const breadcrumbs = [
    ...
    { href: `/${locale}/rent-property-in/${slug}`, label: "Rent" },
  ];
